                ...
                with Manager() as manager:
                    ...
                    for k in range(len(cmat_list)):
                        p = Process(target=run_slice, args=(cmat_list[k],
                                                               ...
                                                               target_locs_list[k], alpha_opt, rho_opt, sn_opt, 1 / precision_vec_list[k], full_pred=False))
                        p.start()
                        processes.append(p)
                    for p in processes:
                        p.join()
...
def run_slice(cmat, convolved_signal, x, target_locs, alpha_opt, rho_opt, sn_opt, prec_vec):
    ...
    mean_f, var_f, x_truncated = gp_deconvolution.pred([cmat], [convolved_signal.flatten()], [x.flatten()],
                                                       [target_locs.flatten()], alpha_opt,
                                                       rho_opt, sn_opt,
                                                       [1 / prec_vec.flatten()], full_pred=False)
